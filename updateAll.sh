...
cat $ARCHIVE_HOME/logs/pids.txt | parallel curl -s -uedoweb-admin:$PASSWORD -XPOST http://localhost:$PLAYPORT/resource/{}/addObjectTimestamp -H"accept: application/json"  >$ARCHIVE_HOME/logs/addObjectTimestamp-`date +"%Y%m%d"`.log 2>&1

echo "lobidify"
cat $ARCHIVE_HOME/logs/titleObjects.txt | parallel curl -s -uedoweb-admin:$PASSWORD -XPOST http://localhost:$PLAYPORT/utils/lobidify/{} -H"accept: application/json"  >$ARCHIVE_HOME/logs/lobidify-`date +"%Y%m%d"`.log 2>&1

echo "enrich"
cat $ARCHIVE_HOME/logs/titleObjects.txt | parallel curl -s -uedoweb-admin:$PASSWORD -XPOST http://localhost:$PLAYPORT/resource/{}/metadata/enrich -H"accept: application/json"  >$ARCHIVE_HOME/logs/enrich-`date +"%Y%m%d"`.log 2>&1
