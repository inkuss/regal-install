...
cat $ARCHIVE_HOME/logs/titleObjects.txt | parallel --jobs 5 ./updatePid.sh {} http://localhost:$PLAYPORT  >$ARCHIVE_HOME/logs/lobidify-`date +"%Y%m%d"`.log 2>&1

echo "index"
cat $ARCHIVE_HOME/logs/pids.txt | parallel --jobs 5 curl -s -uedoweb-admin:$PASSWORD -XPOST http://localhost:$PLAYPORT/utils/index/{} -H"accept: application/json"  >$ARCHIVE_HOME/logs/index-`date +"%Y%m%d"`.log 2>&1
